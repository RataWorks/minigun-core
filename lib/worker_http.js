...
function createRequest(requestSpec, config, ee) {
  ...
  var f = function(context, callback) {
    ...
    if( config.id && config.secret ) {
      var _body = ( requestParams.json ? JSON.stringify( requestParams.json ) : requestParams.body ) || "";
      requestParams = require('api_auth').auth(config.id, config.secret ).sign_options( requestParams, _body);
    }
    ...
    if (match) {
      ...
      if (syntax.body && syntax.body.length === 1 &&
        ...
        if (funcName in context.funcs) {
          return context.funcs[funcName].apply(null, args);
        }
      }
    } else {
