...
function createRequest(requestSpec, config, ee) {
  ...
  var f = function(context, callback) {
    ...
    if( config.id && config.secret ) {
      var body = ( requestParams.json ? JSON.stringify( requestParams.json ) : requestParams.body ) || "";
      
      requestParams = require('api_auth').auth( config.id , config.secret ).sign_options( requestParams , body  );
    }
